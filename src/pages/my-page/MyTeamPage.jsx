...
const MemberRow = styled.div`
  ...
  grid-template-columns: ${(p) =>
    p.isProject ? '56px 1fr 100px 150px auto' : '56px 1fr 100px auto'};
  align-items: center;
  gap: 8px;
  padding: 16px 20px;
  border-bottom: 1px solid ${colors.border};
...
const MyTeamPage = () => {
  ...
  return (
    <Wrap>
      ...
      <Tabs>
        <TabButton data-active={tab === '프로젝트'} onClick={() => setTab('프로젝트')}>
          프로젝트
        </TabButton>
        <TabButton data-active={tab === '과제'} onClick={() => setTab('과제')}>
          과제
        </TabButton>
        <TabButton data-active={tab === '스터디'} onClick={() => setTab('스터디')}>
          스터디
        </TabButton>
      </Tabs>
      ...
      {filtered.map((team) => {
        ...
        const currentUserIsLeader = team.members.some(
          (m) => m.grade === '리더' && m.id === currentUserId
        );
        ...
        <Overlay onClick={closeReview} role="dialog" aria-modal="true">
          <Modal onClick={(e) => e.stopPropagation()}>
            <ModalHeader>
              ...
              <CloseBtn onClick={closeReview} aria-label="닫기">
                ✕
              </CloseBtn>
            </ModalHeader>
            ...
            <Notes>
              <li>
                ...
                내용에 민감한 개인정보가 포함되지 않도록 주의하시기 바랍니다.
              </li>
              <li>
                솔직하게 작성하신 피드백은 다른 분들께 큰 도움이 됩니다. 하지만 허위 사실이나
                명예훼손, 욕설, 타인비방글 등 제 3자의 권리를 침해하는 리뷰는 서비스 이용약관이나
                관련 법률에 따라 제재를 받을 수 있습니다.
              </li>
            </Notes>
